...
This document provides a comprehensive reference for all build rules provided by the Please Rust plugin (`please-rules`).
...
| :--- | :--- | :--- | :--- |
| `name` | `str` | *Required* | Name of the build target. |
...
| Parameter | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `name` | `str` | *Required* | Name of the executable target. |
...
Compiles Rust source files with test configuration (`--test`) and runs the unit/integration tests using Please's test runner, outputting standard JUnit XML results.
...
Fetches and builds third-party crates from `crates.io` using Cargo in an isolated temporary build environment, producing `.rlib` or `.so` artifacts.
...
| Parameter | Type | Default | Description |
| :--- | :--- | :--- | :--- |
| `name` | `str` | *Required* | Rule name in the `BUILD` file. |
