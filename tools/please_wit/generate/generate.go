...
func GenerateFromAST(opts Options, witPath string) error {
	...
	case "go", "golang":
		...
		content := GenerateGoCode(pkg, opts.Package)
		return os.WriteFile(filepath.Join(outDir, filename), []byte(content), 0644)
	...
	case "cpp", "cc", "c", "cxx":
		...
		content := GenerateCppCode(pkg, opts.Package)
		if err := os.WriteFile(filepath.Join(outDir, filename), []byte(content), 0644); err != nil {
			return err
		}
		cppFile := strings.TrimSuffix(filename, filepath.Ext(filename)) + ".cpp"
		cppContent := fmt.Sprintf("// Auto-generated by please_wit from WIT AST. DO NOT EDIT.\n#include \"%s\"\n", filename)
		return os.WriteFile(filepath.Join(outDir, cppFile), []byte(cppContent), 0644)
	}
